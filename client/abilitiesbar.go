...
func DrawAbilities(win *pixelgl.Window, me *model.Player){
	...
	bulletsText := text.New(pixel.V(abilitiesBarPosX-abilitiesBar.Picture().Bounds().Max.X/1.5,abilitiesBarPosY*0.9),basicAtlas)

	fmt.Fprintln(weaponText, model.Weapons[myWep].Name)
	if myWep!=model.KNIFE{
